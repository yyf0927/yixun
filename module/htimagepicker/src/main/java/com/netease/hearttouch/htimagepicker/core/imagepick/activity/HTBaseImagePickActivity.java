...
import com.netease.hearttouch.htimagepicker.core.imagepick.event.InternalUpdateSelectedImagesListener;
 ...
 */
public abstract class HTBaseImagePickActivity extends BaseActivity
        implements ImageScanCompleteListener,
        InternalUpdateSelectedImagesListener,
    ...
    protected String mCurrImageFolderName;
    protected ImageFolder mCurrImageFolder;

    // 当前已经选中的图片列表
    protected List<String> mInitSelectedImagePaths = new ArrayList<>();
...
//                break;
    ...
    public List<Image> getSelectedImages() {
        if (mSelectedImages == null) {
            syncSelectedImagesFromFolder();
        }

        return mSelectedImages != null ? mSelectedImages : new ArrayList<Image>();
    }

    @Override
    final public void onInternalUpdateSelectedImages(final EventUpdateSelectedPhotosModel event) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                List<Image> newSelectedImages = event.getSelectedImages();
                if (newSelectedImages != null) {
                    List<Image> selectedImages = new ArrayList<Image>();
                    for (Image newImage : newSelectedImages) {
                        Image image = ImageHelper.findImage(mCurrImageFolder, newImage.getId());
                        if (image != null) {
                            image.setSelected(newImage.isSelected());
                            selectedImages.add(image);
                        }
                    }

                    updateSelection(selectedImages, false);
                    onUpdateSelectedImagesFromPreview(getSelectedImages());
                }
            }
        });
    }
    ...
    /**
     * 重新扫描全部 image 对象的 select 标志位，确认 mSelectedImages
     */
    protected void syncSelectedImagesFromFolder() {
        ...
        for (Image img : mCurrImageFolder.getImages()) {
            if (img.isSelected()) {
    ...
    private void initSelectedImages() {
        ...
        if (imageFolder != null && imageFolder.size() > 0) {
            ...
            if (mCurrImageFolderName != null) {
                for (ImageFolder folder : imageFolder) {
                    if (TextUtils.equals(folder.getName(), mCurrImageFolderName)) {
                        currFolder = folder;
                        break;
                    }
                }
            ...
            mCurrImageFolderName = mCurrImageFolder != null ? mCurrImageFolder.getName() : null;

            if (mInitSelectedImagePaths.size() > mSelectLimit) {
                mInitSelectedImagePaths = mInitSelectedImagePaths.subList(0, mSelectLimit);
            }

            for (String imagePath : mInitSelectedImagePaths) {
                for (Image img : mCurrImageFolder.getImages()) {
                    if (TextUtils.equals(imagePath, img.getAbsolutePath())) {
    ...
    /**
     * 更新选中的图片
     * @param newImageList  新的选中的图片列表
     * @param bClearOrigin  是否清空原有的选中图片列表
     * @return true：数据更新正常；false：数据更新后，超出被裁剪
     */
    public final boolean updateSelection(List<Image> newImageList, boolean bClearOrigin) {
        if (bClearOrigin) {
            getSelectedImages().clear();
        }

        ImageHelper.updateSelection(newImageList, mSelectedImages);

        if (mSelectedImages.size() > getSelectLimit()) {
            onImageSelectedOverflow(mSelectedImages.size(), getSelectLimit());

            // 重置标签
            int limit = getSelectLimit();
            int imgSize = mSelectedImages.size();
            for (int i=limit; i<imgSize; i++) {
                mSelectedImages.get(i).setSelected(false);
            }

            mSelectedImages = mSelectedImages.subList(0, getSelectLimit());
            return false;
        }
        return true;
    }
     ...
     */
    final public void startPreviewMultiImages(List<Image> images, @Nullable IIntentProcess intentProcess) {
        if (images != null && !images.isEmpty()) {
            ImagePreviewHelper.startMultiImagePreview(this, images, intentProcess);
        }
    }
...
//        }
    ...
    private void processExtra() {
        ...
        if (intent != null) {
            ...
            mCurrImageFolderName = intent.getStringExtra(Extras.EXTRA_IMAGE_FOLDER_NAME);

            mInitSelectedImagePaths = intent.getStringArrayListExtra(Extras.EXTRA_IMAGE_LIST);
            if (mInitSelectedImagePaths == null) {
                mInitSelectedImagePaths = new ArrayList<>();
            }
        }
    }
    ...
    private void callFinishListener(int pickListenerKey, boolean isConfirmUse) {
        ...
        String imageFolderName = mCurrImageFolder != null ? mCurrImageFolder.getName() : "";
        ...
        if (isConfirmUse && mIsCut && mFileOutPath != null) {
            ...
            images.add(image);
            ImagePickerListenerCache.callImagePickerFinishListener(pickListenerKey, imageFolderName, images);
        } else if (isConfirmUse && selectedPhotos.size() > 0) {
            ImagePickerListenerCache.callImagePickerFinishListener(pickListenerKey, imageFolderName, selectedPhotos);
        } else {
