package com.netease.study.exercise.widget.recyclerview;
...
/**
 * 包含展示loading中和空文案显示,如果是出错情况可以复用空文案显示但可以设个点击进行重试
 */
public class NovaRecyclerView extends RecyclerView {
        ...
        addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (mLoadMore) {
                    ...
                    if (!mLoadingMore && (firstVisibleItemPosition + visibleItemCount) >= itemCount - 1 && ((NovaAdapter) getAdapter()).getNormalItemCount() != 0) {
                        load();
                    }
                }
            }
    ...
    private static class LoadView extends RelativeLayout {
        ...
        public LoadView(Context context, int minHeight) {
            ...
            addView(LayoutInflater.from(context).inflate(R.layout.listview_footer_loading, null), layoutParams);
        }
    ...
    public static abstract class NovaAdapter<T, VH extends NovaViewHolder> extends RecyclerView.Adapter<NovaViewHolder> {
        ...
        public NovaAdapter(NovaRecyclerView novaRecyclerView) {
            mRecyclerView = novaRecyclerView;
        }
        ...
        public NovaViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == VIEW_TYPES.EMPTY) {
                ...
                loadItemView.addView(new LoadView(context, context.getResources().getDimensionPixelSize(R.dimen.loadview_height)));
