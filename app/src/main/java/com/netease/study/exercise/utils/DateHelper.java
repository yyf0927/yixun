...
public class DateHelper {
    ...
    public String getRencentTime(String date) {
        ...
        if (curDate.equals(paramDate)) {
            ...
            if (hour == 0) {
                ...
                long tt = cal.getTimeInMillis() - time.getTime();
                if (tt < 60000){
                    ftime = "刚刚";
                }
                else {
                    ftime = Math.max(tt / 60000, 1) + "分钟前";
                }
            }
            else
                ftime = hour + "小时前";
    ...
    {
        String text = date_Formater_1.format(date);
        return getRencentTime(text);
    }
